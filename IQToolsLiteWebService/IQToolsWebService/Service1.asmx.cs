...
namespace IQToolsWebService
{
    ...
    public class Service1 : System.Web.Services.WebService
    {
        ...
        private const string folder = "C:\\IQToolsLite\\";
        ...
        {
            Thread merger = new Thread(() => MergeThread(emr, sql, iqtoolsDB, DBs, UID));
        ...
        public void MergeThread(string emr, string sql, string DB, string[] DBs, int UID)
        {
            ...
            foreach (string dbx in DBs)
            {
                if (emr != "isante")
                {
                    ...
                    try
                    {
                        /* If Query Contains CTE
                         * Drop View viewCTE IF EXISTS
                         * Create a View
                         * Select * FROM that View INTO Normal Tables
                         * Drop View                         
                         */
                        ...
                        {
                            i = (int)en.ReturnObject(varcnnstr, ClsUtility.theParams
                                , "DROP VIEW IF EXISTS viewCTE"
                                , ClsUtility.ObjectEnum.ExecuteNonQuery, "mssql");

                            string viewCTE = "CREATE VIEW viewCTE AS " + sql;
                            i = (int)en.ReturnObject(varcnnstr, ClsUtility.theParams
                                , viewCTE
                                , ClsUtility.ObjectEnum.ExecuteNonQuery, "mssql");

                            i = (int)en.ReturnObject(varcnnstr, ClsUtility.theParams
                                , "SELECT * INTO " + DB + ".dbo.Mgr_" + UID.ToString() + "" + dbx + " FROM viewCTE"
                                , ClsUtility.ObjectEnum.ExecuteNonQuery, "mssql");
                        ...
                        {
                            i = (int)en.ReturnObject(varcnnstr, ClsUtility.theParams
                                , "SELECT * INTO " + DB + ".dbo.Mgr_" + UID.ToString() + "" + dbx + " FROM (" + sql + ")temp"
                                , ClsUtility.ObjectEnum.ExecuteNonQuery, "mssql");
                ...
                else if (emr == "isante")
                {
                    ...
                    try
                    {
                        i = (int)en.ReturnObject(varcnnstr, ClsUtility.theParams, "CREATE TABLE `" + DB + "`.`mgr_" + dbx + "` " + sql + ""
                            , ClsUtility.ObjectEnum.ExecuteNonQuery, "mysql");
            ...
            if (MergeString.Length > 6)
            {
                ...
                if (emr != "isante")
                {
                    ...
                    {
                        i = (int)en.ReturnObject(varcnnstr, ClsUtility.theParams
                            , "SELECT * INTO Mgr_" + UID.ToString() + " FROM (" + toMerge + ")temp"
                            , ClsUtility.ObjectEnum.ExecuteNonQuery, "mssql");
                        DataRow n = (DataRow)en.ReturnObject(varcnnstr, ClsUtility.theParams
                            , "SELECT Count(*) FROM Mgr_" + UID.ToString(), ClsUtility.ObjectEnum.DataRow, "mssql");
                ...
                else if (emr == "isante")
                {
                    ...
                    try
                    {
                        i = (int)en.ReturnObject(varcnnstr, ClsUtility.theParams, "CREATE TABLE mgr_" + UID.ToString() + toMerge + ";"
                            , ClsUtility.ObjectEnum.ExecuteNonQuery, "mysql");
                        DataRow n = (DataRow)en.ReturnObject(varcnnstr, ClsUtility.theParams, "SELECT Count(*) FROM Mgr_" + UID.ToString()
            ...
            if (i != 0)
            {
                ...
                */
                DataTable exDt = (DataTable)en.ReturnObject(varcnnstr, ClsUtility.theParams, "SELECT * FROM Mgr_" + UID.ToString()
                    , ClsUtility.ObjectEnum.DataTable, "mssql");
                string exFileName = folder + "Mgr_" + UID.ToString() + ".xlsx";
                try
                {
                    link = folder + CreateExcel(exDt, exFileName);
                }
                catch (Exception ex) { ErrorMessage += ex.Message; }
                //link = exFileName;
                //}
                ClsUtility.Init_Hashtable();
                DataRow dr = (DataRow)en.ReturnObject(varcnnstr, ClsUtility.theParams
                    , "Select FirstName From aa_Users Where UID = " + UID.ToString()
                    , ClsUtility.ObjectEnum.DataRow, "mssql");
        ...
        /*private string SQLToAccess(string emr, string DB, string table)
        {
            ...
            {
                string varcnnstr = getConnString("mssql") + ";Initial Catalog=" + DB;
                dt = (DataTable)en.ReturnObject(varcnnstr, ClsUtility.theParams
                    , "SELECT * FROM " + table, ClsUtility.ObjectEnum.DataTable, "mssql");
            }
            else if (emr == "isante")
            {
                string varcnnstr = getConnString("mysql") + ";Database=" + DB;
                dt = (DataTable)en.ReturnObject(varcnnstr, ClsUtility.theParams
                    , "SELECT * FROM " + table, ClsUtility.ObjectEnum.DataTable, "mysql");
            }
        ...
        private void EmailIt(string userName, string dbLink, string errors, string toEmail)
        {
            ...
            if (errors == "" && dbLink != "")
            {
                if (dbLink.Contains("xls") || dbLink.Contains("zip"))
                {
                    ...
                    html.WriteEncodedText("IQToolsLite System");
        ...
        public string SaveQuery(string UID, string emr, string sql, string qryName, string qryDesc)
        {
            ...
            try { i = (int)en.ReturnObject(connString, ClsUtility.theParams
                , insertString, ClsUtility.ObjectEnum.ExecuteNonQuery, "mssql"); }
            catch (Exception ex) { return ex.Message; }
            if (i == 1) { return "Success"; }
            else return "Fail";
        }
        ...
        public string UpdateQuery(string UID, string emr, string sql, string qryName, string qryDesc)
        {
            ...
            ClsUtility.Init_Hashtable();
            int i = 0;
            DataRow dr = (DataRow)en.ReturnObject(connString,ClsUtility.theParams
                ,"SELECT TOP 1 qryID FROM aa_queries where qryName = '"+qryName + "' AND UID = " + UID + ""
                ,ClsUtility.ObjectEnum.DataRow,"mssql");
        ...
        public void LogUserHistory(int UserID, string logInOut)
        {
            ...
            try
            {
                int i = (int)en.ReturnObject(varcnnstr, ClsUtility.theParams, updateString
                    , ClsUtility.ObjectEnum.ExecuteNonQuery, mainServerType);
            }
            catch (Exception ex) {
        ...
        private string CreateExcel(DataTable dt, string fName)
        {
            var rowCount = dt.Rows.Count;
            if (File.Exists(fName))
                File.Delete(fName);

            var newFile = new FileInfo(fName);
            using (var package = new ExcelPackage(newFile))
            {
                ExcelWorksheet worksheet = package.Workbook.Worksheets.Add("IQToolsLite");
