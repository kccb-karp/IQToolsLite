        ...
        private void RestoreDB(string emr, string[] baks)
        {
            ...
            if (emr == "iqcare" || emr == "icap")
            {
                foreach (string bak in baks)
                {
                    ...
                    try
                    {
                        i = (int)en.ReturnObject(clsGbl.cnnstr, ClsUtility.theParams
                    ...
                    restoreString = "RESTORE DATABASE " + dbName + " " +
                                    "FROM DISK = N'" + bak + "' " +
                                    "WITH FILE = 1, " +
                                    "MOVE N'TestDataBase_IQCare' TO N'" + clsGbl.dataFolderPath + "\\" + dbName + ".mdf' , " +
                                    "MOVE N'TestDataBase_IQCare_log' TO N'" + clsGbl.dataFolderPath + "\\" + dbName + "_log.ldf'";
                    try
                    {
        ...
        private void RestoreIQToolsDB(string emr, string db)
        {
            ...
            if (emr != "isante")
            {
                string restoreString = "RESTORE DATABASE IQTools_" + db.Trim() + " \n" +
                                        "FROM DISK = 'C:\\IQToolsLite\\IQTools.bak' " +
                                        "WITH REPLACE, " +
                                        "MOVE 'IQTools' TO '" + clsGbl.dataFolderPath + "\\IQTools_" + db.Trim() + ".mdf' , " +
                                        "MOVE 'IQTools_log' TO '" + clsGbl.dataFolderPath + "\\IQTools_" + db.Trim() + "_log.ldf'";
            ...
            if (emr != "isante")
            {
                ...
                try
                {
                    ...
                    if (emr == "iqcare")
                    {
                        try
                        {
                            i = (int)en.ReturnObject(iqtConnString, ClsUtility.theParams, "DROP TABLE mst_patient_decoded", ClsUtility.ObjectEnum.ExecuteNonQuery, "mssql");
                        }
                        catch (Exception ea) { 
